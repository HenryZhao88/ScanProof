...
.PHONY: help setup data train audit demo build serve dev test clean-artifacts reproduce
...
help:
	...
	@echo "  make reproduce  data -> train -> audit -> demo -> build"
...
reproduce: data train audit demo build
	@echo "full pipeline complete — run 'make serve'"

clean-artifacts:
	rm -f artifacts/audit_cases.json
